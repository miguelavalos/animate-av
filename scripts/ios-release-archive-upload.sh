...
team_id="${ANIMATEAV_APPLE_TEAM_ID:-935PM55U6R}"
...
Reproducible Animate AV iOS release workflow:
1. validates generated prod config;
...
xcconfig_get() {
  local key="$1"
  awk -F= -v key="$key" '$1 ~ "^[[:space:]]*" key "[[:space:]]*$" {gsub(/^[[:space:]]+|[[:space:]]+$/, "", $2); print $2; exit}' \
...
xcconfig_set() {
  ...
  for file in "$repo_root/apps/ios/Config/Debug.xcconfig" "$repo_root/apps/ios/Config/Release.xcconfig"; do
    /usr/bin/perl -0pi -e "s/^${key}[[:space:]]*=.*$/${key} = ${value}/m" "$file"
  done
}
...
if [ "$use_existing_archive" -eq 0 ]; then
  ...
  app_info="$archive_path/Products/Applications/AnimateAV.app/Info.plist"
...
if [ -z "$archive_path" ]; then
  ...
  archive_path="$repo_root/.derived-data/release-archives/AnimateAV-${version_number}-${build_number}-${timestamp}.xcarchive"
fi

mkdir -p "$(dirname "$archive_path")"

if [ "$skip_preflight" -eq 0 ] && [ "$use_existing_archive" -eq 0 ]; then
  run_step "Run iOS Release/prod runtime config check"
  "$repo_root/scripts/check-ios-runtime-config.sh" --env prod --configuration Release
fi

if [ "$use_existing_archive" -eq 0 ]; then
  run_step "Archive signed iOS release"
  xcodebuild archive \
    -project "$repo_root/apps/ios/AnimateAV.xcodeproj" \
    -scheme AnimateAV \
    ...
    AVALSYS_APPLE_DEVELOPMENT_TEAM="$team_id"
else
...
ANIMATEAV_APPLE_TEAM_ID="$team_id" "$repo_root/scripts/check-ios-release-archive.sh" \
...
if [ "$upload" -eq 1 ]; then
  ...
  xcodebuild -exportArchive \
    -archivePath "$archive_path" \
    -exportPath "$repo_root/.derived-data/release-uploads/AnimateAV-${version_number}-${build_number}" \
...
To upload this exact archive, rerun:
  scripts/ios-release-archive-upload.sh --archive "$archive_path" --upload --skip-preflight
REPORT
fi
