...
usage() {
  cat <<'USAGE'
Usage:
  scripts/install-ios-device.sh --env dev|staging|prod [--device <UDID>] [--configuration Debug|Release] [--development-team <TEAM_ID>] [--no-launch]

Regenerates Animate AV iOS Local.xcconfig for the selected environment,
validates the effective Xcode settings, builds, installs on a connected iPhone,
and launches the installed bundle.

Use --development-team, or DEVELOPMENT_TEAM in the environment, when Xcode UI can
sign the app but xcodebuild cannot resolve the local Apple account/team.
USAGE
}
...
app_path="$derived_data_path/Build/Products/$configuration-iphoneos/AnimateAV.app"
...
echo "Installed Animate AV $env_name on device $device_udid as $bundle_identifier."
